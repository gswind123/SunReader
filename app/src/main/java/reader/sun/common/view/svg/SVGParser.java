...
public class SVGParser {
	...
	/**
	 * 通过XMLReader解析data，并设置SVGHandler为监听器。
	 * handler负责解析的过程中绘制canvas，并用Picture对象来记录canvas的绘制路径。
	 * 然后通过Picture新建SVG对象，并返回。
	 */
	 ...
	 */
	/**
	 * 通过读取SVG XML文档中<path></path>标签下的内容，s
	 * 来绘制轨迹，记录在Path类中。绘制轨迹的规则如上面英文注释所示。
	 * */
	...
	//通过name从attrbutes中查找NumberParse对象
...
*/
	...
	/**
	 * SVGParser解析SVG的时候会调用SVGHandler中的函数
	 * 
	 * Picture对象是用来记录Canvas绘制的路径的。
	 * */
	static class SVGHandler extends DefaultHandler {
		...
		private boolean doFill(Properties atts, RectF bounding_box) {
			...
			if (fillString != null) {
				...
				if (fillSet) {
					...
					fillPaint.setShader(null);
					if (searchColor != null && searchColor == -1 && replaceColor!=null) {
						fillPaint.setColor(replaceColor);
					}
					else {
        ...
        private void doColor(Properties atts, Integer color, boolean fillMode, Paint paint) {
			...
			else if (searchColor != null && searchColor == -1 && replaceColor!=null) {
				c = replaceColor;
			}
